...
def get_next_token(
    model: MLModel, prompt_tokens: np.ndarray
    ...
    def inference(
        model: MLModel, input_ids: np.ndarray, num_past_tokens: int
    ) -> np.ndarray:
...
def generate(
    ...
    for i, token in enumerate(
        get_next_token(model, prompt_tokens=prompt_tokens.astype(np.int32))
    ):
